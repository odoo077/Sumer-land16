...
class Partner(models.Model):
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if 'user_id' in values and values.get('user_id') and 'salesperson_ids' in values:
                if values['salesperson_ids'] == [(6, 0, [])]:
                    values['salesperson_ids'] = [(6, 0, [values['user_id']])]
            if 'user_id' in values and values.get('user_id') and not 'salesperson_ids' in values:
                values['salesperson_ids'] = [(6, 0, [values['user_id']])]
