...
class PurchaseOrderLine(models.Model):
    ...
    def _get_stock_move_price_unit(self):
        price_unit_old = super(PurchaseOrderLine,self)._get_stock_move_price_unit()
        price = self._get_discounted_price_unit()
        if price != price_unit_old:
            price_unit = price
        else:
...
#         return price
    ...
    def _apply_value_from_seller(self, seller):
        ...
        if not seller:
            if self.order_id.partner_id:
                if self.order_id.partner_id.default_supplierinfo_discount > 0:
    ...
    def _prepare_purchase_order_line_from_seller(self, seller):
        ...
        if not seller:
            if self.order_id.partner_id:
                if self.order_id.partner_id.default_supplierinfo_discount > 0:
                    return {"discount":self.order_id.partner_id.default_supplierinfo_discount}
            return {}
