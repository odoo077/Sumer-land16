...
class AccountPayment(models.Model):
    ...
    def _onchange_partner_id(self):
        ...
        moves = self.env['account.move'].sudo().search(
            [('partner_id', '=', self.partner_id.id), ('company_id', '=',self.company_id.id), ('state', '=', 'posted'),
    ...
    def action_post(self):
        ...
        for record in self:
            liquidity_lines,counterpart_lines,writeoff_lines=record._seek_for_lines()
            payment_balance=abs(sum(counterpart_lines.mapped('balance')))
            move_lines = record.env['account.move.line']
            rec_lines = record.reconcile_invoice_ids.filtered(lambda x: x.amount_residual > 0)
            if rec_lines and payment_balance > 0:
                ...
                for line in rec_lines:
                    ...
                    if invoice_move and payment_move and len(rec_lines) > 0:
                        ...
                        print(('payment_balance %',counterpart_lines))
                        if record.partner_type == 'customer' and payment_balance > 0:
                            pay_balance = 0
                            if payment_balance >= abs(line.amount_residual):
                                pay_balance = abs(line.amount_residual)
                                payment_balance -= abs(line.amount_residual)
                            else:
                                pay_balance = payment_balance
                                payment_balance -= payment_balance
                            rec = self.env['account.partial.reconcile'].create({
                                'amount': pay_balance,
                                'debit_amount_currency': pay_balance,
                                'credit_amount_currency': pay_balance,
                        ...
                        #         'debit_amount_currency': abs(line.amount_residual),
                        #         'credit_amount_currency': abs(line.amount_residual),
