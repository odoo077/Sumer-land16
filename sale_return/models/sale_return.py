...
class Picking(models.Model):
    ...
    return_sale_id  = fields.Many2one("sale.order.return")
...
class SaleOrder(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            ...
            if order.id:
                ...
                invoices = self.env["account.move"].search([("return_sale_id","=",order.id)])
    ...
    def _get_invoice_status(self):
        ...
        line_invoice_status_all = [
            ...
            for d in self.env['sale.order.return.line'].read_group([
    ...
    order_line = fields.One2many('sale.order.return.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    transaction_ids = fields.Many2many('payment.transaction', 'sale_order_transaction_rel_return', 'sale_order_id', 'transaction_id',
    ...
    tag_ids = fields.Many2many('crm.tag', 'sale_order_tag_rel_return', 'order_id', 'tag_id', string='Tags')
    ...
    def action_view_trnasfer(self):
        return {
            'name': ('Trnasfer'),
            'view_mode': 'tree,form',

            'res_model': 'stock.picking',
            'domain': [('return_sale_id', '=', self.id)],
    ...
    def onchange_partner_id(self):
        ...
        if self.env.user.has_group(
                 'product.group_discount_per_so_line') and self.partner_id and self.enable_default_discount:
            values['default_discount'] = self.partner_id.default_discount
        else:
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == _('New'):
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('sale.order.return', sequence_date=seq_date) or _('New')
...
- You should modify the invoicing policy of your product: Open the product, go to the "Sales tab" and modify invoicing policy from "delivered quantities" to "ordered quantities".
    ...
    def _get_invoiceable_lines(self, final=False):
        ...
        return self.env['sale.order.return.line'].browse(invoiceable_line_ids + down_payment_line_ids)
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            down_payments = order.env['sale.order.return.line']
        ...
        if len(invoice_vals_list) < len(self):
            SaleOrderLine = self.env['sale.order.return.line']
    ...
    def _action_cancel(self):
        ...
        inv.button_cancel()
        picking_ids = self.env['stock.picking'].search([('return_sale_id','=',self.id)])
    ...
    def _action_confirm(self):
        ...
        self.create_invoice(picking_id)
        # self.invoice_count+=1

        return True

    def create_invoice(self,picking_id):
        lines=[]
        for stock in self.order_line:
            lines.append((0, 0, {
                'product_id': stock.product_id.id,
                'quantity': stock.product_uom_qty,
                'price_unit': stock.price_unit,
                'product_uom_id': stock.product_uom.id,
                'discount':stock.discount,
                'tax_ids':[(4,tax.id)for tax in stock.tax_id] if stock.tax_id else [],
            }))
