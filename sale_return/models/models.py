...
class SaleOrderLine(models.Model):
    ...
    order_id = fields.Many2one('sale.order.return', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    name = fields.Text(string='Description', required=True)
    sequence = fields.Integer(string='Sequence', default=10)

    invoice_lines = fields.Many2many('account.move.line', 'sale_order_line_invoice_rel_return', 'order_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    ...
    def product_id_change(self):
        ...
        if self.product_id and self.env.user.has_group('product.group_discount_per_so_line'):
             vals['discount'] = self.order_id.default_discount
        else:
            vals['discount'] = 0
